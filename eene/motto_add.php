...
if ( @mysql_query($sql_put_motto) ) {
	...
	header("Location: http://" .$_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/motto.php?success=true");
	exit;	
} else {
	header("Location: http://" .$_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/motto.php?success=false");
	exit;
}
?>